...
| **ARCH-7** ★★ `[R][C]` | **Edition independence (2014 vs 2024).** | 2014 species grant ASIs, classes are known-vs-prepared, backgrounds differ. Mixing silently double-applies or mis-applies. | *(Re-scoped 2026-07-10, see §13.1.)* One edition per campaign; the data addon dictates the rules. Every compendium record is tagged `edition`; the provider ships a `ruleset` record (constants + capability flags) the engine resolves with per-constant fallback to its built-in 2024 defaults. Structural 2014 shapes (species ASIs, spells-known) gate on record-field *presence*, never on the edition string. |
...
| **FE-4** ✅ `[S]` | **Swappable choices on level-up — DONE (spell swap w/ history; others via whole-state editing).** | — | **Spells:** a '🔄 Swap a spell' button + floating picker records each swap in a persisted `spellSwaps` log (out → in @ level), shown as a linked history with ✕-to-forget (B4.5 `058983f`). **Maneuvers/invocations/metamagic/cantrips/fighting-style:** the whole-state editor keeps every pick freely re-editable at any level (FE-7), so a "swap" is just editing the box — no restrictive one-swap-per-level mechanic (per maintainer). |
...
| **FE-7** ✅ `[S]` | **Option-pool picks are unique (DONE — B4.1).** A pool choice granting N picks (Metamagic / maneuvers / invocations / expertise / skills), incl. the level-growing `countByLevel` picker, no longer allows the same option in two boxes. | — | `renderDescriptor` excludes options taken in sibling boxes; `resolveChoices` de-dupes across boxes; a value duplicating an earlier box renders empty to be re-picked. Every box stays freely editable, so a level-up "swap" is just editing the box — corrections are never locked out (per maintainer). |
...
| **SB-6** ★ `[R][C]` | **ED — 2014 carryovers.** Variant Human / Custom Lineage grant a L1 feat + ASIs; 2014 species grant ASIs. | If 2014 content is ever loaded, these conflict with the 2024 no-species-ASI rule (ARCH-7). | *(Re-scoped 2026-07-10, see §13.1.)* A 2014 species record carries `abilityIncreases`; the engine applies it when the field is present (capability `speciesAsi` on the ruleset record gates the Builder UI). 2024 records never carry the field, so 2024 behavior is untouched by construction. |
...
```jsonc
{
  ...
  // ⚠ DESIGN-TIME PROPOSAL — the SHIPPED blob differs: `preparedSpells` and
  // `cantrips` (both keyed by classId) live at the TOP level of the blob, and
  // manual/copied entries live in a top-level `spells[]` with an `origin`
  // field. `model.js` + entry.js are the truth for stored field names.
  "spells": {                                           // §1
...
1. **Edition scope** — ~~**2024-only.**~~ **Amended 2026-07-10: one sheets addon, swappable rules provider.** The SAME sheets addon serves any edition; the installed data addon dictates the system rules via a machine-readable `ruleset` record (constants + capability flags), and the engine's built-in 2024 values become per-constant fallback defaults. One edition per campaign — coexistence of two compendiums in one install stays out of scope (host wiki-kind/route namespaces are first-wins global). `ARCH-7`/`SB-6` are back in scope under this model; the `ruleset:'2024'` blob tag becomes the provider-selection key. Edition differences classify three ways:
   - **Data constants** (~10: multiclass slot table, pact ladder, caster-fraction rounding, ASI budgets + levels, ability caps, point-buy, scroll-copy cost, spellbook allotment, long-rest hit-dice) → `constants.*` on the ruleset record;
   - **Capability flags** (weapon mastery, epic boons, background-vs-species ASI, origin feats) → `capabilities.*` gate engine paths and Builder UI blocks;
   - **Structural gaps** (2014 species ASIs/subraces, spells-known caster model, 2014 feat model) → record-shape extensions + engine branches gated by field *presence*, deferred until a 2014 compendium actually starts.
